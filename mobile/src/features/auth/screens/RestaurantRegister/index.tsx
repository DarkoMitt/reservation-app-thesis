    ...
    <KeyboardAvoidingView
      ...
      <ScrollView contentContainerStyle={styles.container} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.form}>
          ...
          />
          {errors.restaurantName ? <Text style={styles.error}>{errors.restaurantName}</Text> : null}
          ...
          />
          {errors.streetAddress ? <Text style={styles.error}>{errors.streetAddress}</Text> : null}
          ...
          <TouchableOpacity
            ...
            <Text style={form.restaurantType ? styles.dropdownText : styles.dropdownPlaceholder}>
              {form.restaurantType || 'Restaurant type'}
            </Text>
            <Text style={styles.dropdownIcon}>{isRestaurantTypeOpen ? '▲' : '▼'}</Text>
          </TouchableOpacity>
          {errors.restaurantType ? <Text style={styles.error}>{errors.restaurantType}</Text> : null}
          ...
          <TouchableOpacity
            style={[styles.dropdownButton, !form.restaurantType && styles.disabledDropdown]}
            activeOpacity={0.85}
            disabled={!form.restaurantType}
            onPress={() => setIsCuisineTypeOpen(prev => !prev)}>
            <Text style={form.cuisineType ? styles.dropdownText : styles.dropdownPlaceholder}>
              {form.cuisineType || 'Cuisine type'}
            </Text>
            <Text style={styles.dropdownIcon}>{isCuisineTypeOpen ? '▲' : '▼'}</Text>
          </TouchableOpacity>
          ...
          />
          {errors.maxGuests ? <Text style={styles.error}>{errors.maxGuests}</Text> : null}

          <TextInput
            style={styles.input}
            placeholder="Working hours, e.g. 09:00 - 00:00"
            placeholderTextColor="#9A8C7F"
            value={form.workingHours}
            onChangeText={value => handleChange('workingHours', value)}
          />
          {errors.workingHours ? <Text style={styles.error}>{errors.workingHours}</Text> : null}
          ...
          <TextInput
            ...
            onChangeText={value => handleChange('businessRegistrationNumber', value)}
          />
